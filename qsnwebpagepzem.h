#ifndef QSNWEBADAPTERPZEM_H
#define QSNWEBADAPTERPZEM_H
...
class QsnWebPagePZEM : public QsnWeb
{
    Q_OBJECT
public:
    QsnWebPagePZEM(quint32 iID, QsnGlobalModules *modules, QObject *parent = Q_NULLPTR);
...
private:
   // QSnPZEM PZEM;
    ...
    //Energy calculation
    qint64 countpower_power;
    qint64 countpower_msec;
    qint64 countpower_begin;
    int countpower_count;

    //Energy send
    QDateTime powersend_sendtime;
    quint8 sendPowerTimer;
    quint8 sendCurrentTimer;
    quint8 sendVoltageTimer;
    ...
    void sendCmd8(quint8 cmd, quint16 rAddr, quint16 val, quint16 slave_addr = 0xFFFF);
    quint16 CRC16(QByteArray *data, int ld = 0);
    QString deviceState();
    void counterReset();
    ...
    void setPF(float powerfactor);
    void sendEnergy();
    void sendPowerFailure();
    void sendPowerOn();
    void sendPower();
    void sendVoltage();
    void sendCurrent();
